...
address = web3.toChecksumAddress("0xD143761b13c31BC1E960C978A2A770beCE4568CF")


account_2 = "0xd1729baA56A67D67947C36955E53FC5be2D2fE17"  # Fill me in
...
def userLogin():
    ...
    myquery = {"userName": userName,
               "password": password, "approval": "ADMINAPPROVED"}
...
def userRaiseComplaint():
    ...
    raw = ast.literal_eval(json.dumps(data))
    dataUserID = str(uuid.uuid4())
    complaintColl.insert_one(
        {"complaintID": dataUserID, "complaintDesc": raw["complaintDesc"]}
    )
    return {"success": True}
...
def payFine():
    ...
    tx = {
        "nonce": nonce,
        "to": account_2,
        "value": web3.toWei(raw["fineAmount"], "ether"),
        "gas": 2000000,
...
def createFine():
    ...
    string = str(b)
    var = (
        "Fine Has been registerd due to Traffic Rules Violation\nUser name: "
        + data["userName"]
        + "\n"
        + "Fine Reason: "
        + data["fineReason"]
